...
class BalanceChart<T extends Account> extends ConsumerStatefulWidget {
  ...
  const BalanceChart({super.key, this.account, required this.timeRange, this.chartHeight = 400});
...
class _BalanceChartState<T extends Account> extends ConsumerState<BalanceChart> {
  ...
  List<BalanceChartData<DateTime, int>> toChartSeries(List<Transaction> filterTransaction) {
    ...
    // if (groupByDateData.keys.length < widget.timeRange.duration) {
    //   final rangeDates = widget.timeRange.getRangeDates();
    ...
    //       groupByDateData[date] = 0;
    //     }
    //   }
    // }
    // var chartData = groupByDateData.entries
    //     .map((e) => BalanceChartData<DateTime, int>(
    //           x: e.key,
    //           y: e.value.isEmpty
    //               ? 0
    //               : e.value.map((transaction) => transaction.amount.abs()).reduce(
  ...
  List<Transaction> filterTransaction(Iterable<Transaction> values) {
    return values
        ...
        .where((transaction) => widget.timeRange.contain(transaction.timestamp))
        .toList();
  }
